...
wilcox.test(ot_summary_wide_acc$Day_Block_1_3, ot_summary_wide_acc$Day_Block_3_6, paired = TRUE, alternative = "two.sided", conf.int = TRUE)
...
# overtraining table with 
ot_summary_wide_acc$diff <- ot_summary_wide_acc$Day_Block_3_6-ot_summary_wide_acc$Day_Block_1_3
ot_summary_wide_sdrt$diff <- ot_summary_wide_rt$Day_Block_1_3-ot_summary_wide_rt$Day_Block_3_6
ot_summary_wide_sdrt$diff <- ot_summary_wide_rt$Day_Block_1_3-ot_summary_wide_sdrt$Day_Block_3_6
...
qqnorm(resid(Fit5_noWML))
hist()
# (pseudo) effect sizes 
r2.corr.mer(Fit5_noWML)
eff <- 1-var(residuals(Fit5_noWML))/var(model.response(model.frame(Fit5_noWML)))
...
# accuracy
Fit5 = glmer(OptimalChoice ~ (1|Subject)+Congruency*Group*WM_Load, data=RL_alldeval, family=binomial) # 
Anova(Fit5)
summary(Fit5)
...
# RT
FitRT = lmer(RT ~ (1|Subject)+Congruency*Group*WM_Load, data=filter(RL_alldeval, OptimalChoice==1)) # 
Anova(FitRT)
summary(FitRT)
...
# accuracy
Fit5_WML = glmer(OptimalChoice ~ (1|Subject)+Congruency*Group, data=RL_deval_wm, family=binomial) #
summary(Fit5_WML)
Anova(Fit5_WML)
testInteractions(Fit5_WML, pairwise=c("Congruency","Group"))
qqnorm(resid(Fit5_WML))
# RT
FitRT_WML = lmer(RT ~ (1|Subject)+Congruency*Group, data=filter(RL_deval_wm, OptimalChoice==1)) #
summary(FitRT_WML)
Anova(FitRT_WML)
...
qqnorm(resid(FitRT_WML))
hist(a$RT)
a=filter(RL_deval_wm, OptimalChoice==1)
...
if (write_it==TRUE){
  ggsave(file="wm_bar_loads.svg", plot=bar_wmload, width=3.7, height=6)
  ggsave(file="wm_boxplot_loads.svg", plot=boxplot_wmload, width=3.7, height=6)
  ggsave(file="wm_bar_performance.svg", plot=bar_wm_perform, width=4, height=6)
...
#   theme(plot.title=element_text(colour="Black"))
# rlwm_acc_devalpoint
rl_accuracy_betweenerrorbars$Group2 <- factor(rl_accuracy_betweenerrorbars$Group, levels = c("CT","EOT","COT"))
rl_accuracy_betweenerrorbars$Congruency2 <- factor(rl_accuracy_betweenerrorbars$Congruency, levels = c("Valued","Devalued"))
...
rldeval_rtg$Group2 <- factor(rldeval_rtg$Group, levels = c("CT","EOT","COT"))
rldeval_rtg$Congruency2 <- factor(rldeval_rtg$Congruency, levels = c("Valued","Devalued"))
RL_alldeval$Group2<-factor(RL_alldeval$Group, levels = c("CT","EOT","COT"))
RL_alldeval$Congruency2<-factor(RL_alldeval$Congruency, levels = c("Valued","Devalued"))
rlwm_rt_devalpoint <- ggplot(rldeval_rtg, aes(x=Group2, y=RT, group=Congruency2)) + #scale_y_continuous(limits = c(0.4, 1.1))+
  geom_violin(data=RL_alldeval, aes(x=Group2, y=RT, group=Congruency2, colour=Group2), position=position_dodge(.4))+
